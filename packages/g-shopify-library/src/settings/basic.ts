 ...
 */
export const basic_settings_schema_map= {
    ...
    radio:  zod.object({
        ...
        default: zod.union([zod.string(), zod.optionalDefault( zod.number(),0)]) ,
    }),
    select: zod.object({ //todo: make this dynamic or factory??
    ...
    range:  zod.object({
        ...
        /* * unit - example: "px" * */
        unit:  z.union( [ z.enum( ["px","%","em","rem","vh"]) ,zod.optionalDefault( z.string(),"px")] )
    }),
}
export type BasicSetting<Type extends keyof typeof basic_settings_schema_map> =   z.infer<typeof basic_settings_schema_map[ Type ]>

 const defaultSchemaType = zod.object({
    type: z.literal('range'),
})

export enum BasicSettingsTyoes {
    Text = "text",
    TextArea = "textarea",
    Number = "number",
    Checkbox="checkbox",
...
const basic_SettingsEnum = z.nativeEnum(BasicSettingsTyoes);
